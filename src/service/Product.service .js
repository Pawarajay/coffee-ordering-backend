...
const productService = {
  ...
  async getList(query) {
    ...
    if (query.is_available_kiosk !== undefined) { conditions.push('p.is_available_kiosk = ?'); params.push(query.is_available_kiosk ? 1 : 0); }
    if (query.is_available_d2c !== undefined)   { conditions.push('p.is_available_d2c = ?');   params.push(query.is_available_d2c ? 1 : 0); }
    if (query.search) {
  ...
  async update(uuid, data) {
    ...
    for (const [col, val] of booleanMappings) {
    ...
    if (data.meta !== undefined) {
      fields.push('meta = ?'); values.push(data.meta ? JSON.stringify(data.meta) : null);
    }
